...
const PaginaDetalle = () => {
  ...
  const personajeDetalle: Personaje | undefined = personajes.personajes.find(
    (per) => per.id === id
  );
  ...
  useEffect(() => {
    if (personajeDetalle) {
      const array: (string | undefined)[] = personajeDetalle.episodio.map(
        (episodio) => {
    ...
    <div className="container">
      ...
      <div className={"detalle"}>
        <div className={"detalle-header"}>
          ...
          <BotonFavorito
            esFavorito={getFavorito(personajeDetalle.id)}
            onClick={(e) => handleClickFavorito(personajeDetalle.id)}
          />
        </div>
      </div>
