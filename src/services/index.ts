...
export const fetchEpisodios = async (
  ...
  const res = await axios.get(
    `https://rickandmortyapi.com/api/episode/${numbers}`
  );
...
export const getPersonajes = async (
  ...
  const res = await axios.get<AxiosPersonajeResponse>(
    `https://rickandmortyapi.com/api/character/?page=${pages}&name=${name}`
  );
